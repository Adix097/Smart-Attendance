...
export default function WebcamCapture({
  ...
  useEffect(() => {
    ...
    if (source?.type !== 'webcam' || !element) return;
    element.srcObject = source.stream;
    return () => {
      if (element.srcObject === source.stream) element.srcObject = null;
    };
  }, [source]);
    ...
    <div className="grid gap-3">
      {source?.type === 'webcam' ? (
        <>
          ...
          <button
            type="button"
            className="w-fit rounded-lg border border-slate-300 px-4 py-2 hover:bg-slate-50"
            onClick={stop}
            disabled={disabled}
          >
            Stop camera
        ...
        <button
          type="button"
          className="w-fit rounded-lg bg-blue-700 px-4 py-2.5 font-medium text-white hover:bg-blue-800 disabled:opacity-50"
          onClick={start}
          disabled={disabled}
        >
          Start camera
